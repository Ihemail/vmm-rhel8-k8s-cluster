#!/bin/bash
 ...
 sed -i '/subscription-manager/d' /root/k8s-docker-ce-ha-master-1.sh
 sudo sed -i 's/reboot now#/#reboot now/g' /root/k8s-docker-ce-ha-master-1.sh
 #reboot now#
...
echo "    --pod-network-cidr=10.244.0.0/16 --cri-socket /run/cri-dockerd.sock"
sudo kubeadm init --control-plane-endpoint="$master_ip:6443" --upload-certs \
  --apiserver-advertise-address=$eth0ip \
  --pod-network-cidr=10.244.0.0/16 \
  --cri-socket /run/cri-dockerd.sock    ## --kubernetes-version 1.24.6
