#!/bin/bash
 ...
 sed -i '/subscription-manager/d' /root/k8s-crio-ha-master-2.sh
 sudo sed -i 's/reboot now#/#reboot now/g' /root/k8s-crio-ha-master-2.sh
 #reboot now#
...
kubeadm join $master_ip:6443 --token $master_token --discovery-token-ca-cert-hash $master_token_hash \
  --control-plane --certificate-key $cert_key --cri-socket=unix:///run/crio/crio.sock
echo "kubeadm join $master_ip:6443 --token $master_token --discovery-token-ca-cert-hash $master_token_hash \ "
echo "    --control-plane --certificate-key $cert_key --cri-socket=unix:///run/crio/crio.sock"
